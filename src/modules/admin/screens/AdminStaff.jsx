...
function InviteForm({ schoolId, onSuccess, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // admin-create-user derives school_id from the caller's own profile
      // server-side — we deliberately don't send schoolId from the client,
      // since a compromised or buggy client could otherwise specify an
      // arbitrary school. See supabase/functions/admin-create-user/index.ts.
      ...
      if (fnErr) {
        ...
        // this helper re-reads it from error.context to get our actual
        // "A user with this email already exists" style message instead
        // of the generic "non-2xx status code" text.
        throw new Error(await extractEdgeFunctionErrorMessage(fnErr));
      }
      ...
      <div className="space-y-1.5 text-[12px] mb-4">
        {member.email && (
          <div className="flex items-center gap-2 text-[var(--c-ink-3)]">
            <Icon name="mail" className="text-[14px] text-[var(--c-ink-4)]" />
            <span className="truncate">{member.email}</span>
          </div>
        )}
        {member.phone && (
          <div className="flex items-center gap-2 text-[var(--c-ink-3)]">
            <Icon name="phone" className="text-[14px] text-[var(--c-ink-4)]" />
            {member.phone}
          </div>
        )}
        <div className="flex items-center gap-2 text-[var(--c-ink-3)]">
          <Icon name="chalkboard" className="text-[14px] text-[var(--c-ink-4)]" />
          {classes.length > 0
