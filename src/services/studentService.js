...
export async function getNextLesson(pupilId) {
  ...
  return data?.[0] ?? null;   // RPC returns a set; we take the first (and only) row
}
...
export async function startLesson(pupilId, lessonId) {
  const { error } = await supabase
    ...
    .upsert(
      ...
      { onConflict: 'pupil_id,lesson_id', ignoreDuplicates: true },
    );

  if (error && error.code !== '23505') {
    // 23505 = unique_violation, which means the row already exists. Fine.
    console.warn('[studentService] startLesson upsert failed:', error.message);
  }
}
