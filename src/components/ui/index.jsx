...
// ── KPI Card ─────────────────────────────────────────────────────────────────
export function KpiCard({ label, value, delta, deltaDir = 'flat', footnote, isLoading, icon, href, onClick }) {
  const deltaColors = {
  ...
  const inner = (
    <div className={cn(
      'bg-[var(--c-surface-2)] border border-[var(--c-line-2)] rounded-xl p-5 relative overflow-hidden',
      interactive && 'cursor-pointer hover:border-[var(--product-accent)] hover:bg-[var(--c-surface-3)] transition-all group',
    )}>
      {icon && (
        <div className="absolute top-4 right-4 w-9 h-9 rounded-lg flex items-center justify-center opacity-20 group-hover:opacity-40 transition-opacity"
          style={{ background: 'var(--product-accent)' }}>
          <Icon name={icon} className="text-[18px]" style={{ color: '#1a1305' }} />
        </div>
      )}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center gap-3">
      <div className="w-12 h-12 rounded-full bg-[var(--c-surface-3)] flex items-center justify-center">
        <Icon name={icon} className="text-[24px] text-[var(--c-ink-4)]" />
      </div>
...
export function Alert({ type = 'info', children }) {
  ...
  return (
    ...
    >
      <Icon name={s.icon} className="text-[18px] mt-0.5 shrink-0" style={{ color: s.color }} />
