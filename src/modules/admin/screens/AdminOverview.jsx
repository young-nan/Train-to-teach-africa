...
// Alert row
function AlertRow({ type = 'error', icon, title, sub }) {
  const colors = {
  ...
  return (
    <div
      className="flex gap-3 p-3 rounded-lg mb-2 last:mb-0"
      style={{ background: c.bg, borderLeft: `3px solid ${c.border}` }}
    >
      <i className={`ti ti-${icon} text-[17px] mt-0.5 shrink-0`} style={{ color: c.ic }} aria-hidden="true" />
      <div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 mb-6">
        <KpiCard label="Students enrolled" value={data?.studentCount ?? '—'} delta="+4 this term" deltaDir="up" icon="users" />
        <KpiCard label="Teaching staff"    value={data?.staffCount ?? '—'}   delta="2 on leave"   deltaDir="flat" icon="id-badge-2" />
        <KpiCard label="Avg attendance"    value={avgAtt ? `${avgAtt}%` : '—'} delta="+2% vs last week" deltaDir="up" icon="calendar-stats" />
        <KpiCard label="Classes"           value={data?.classes?.length ?? '—'} deltaDir="flat" icon="chalkboard" />
        <KpiCard label="Interventions"     value={interventionCount}          delta={interventionCount > 0 ? 'Needs attention' : 'All clear'} deltaDir={interventionCount > 0 ? 'down' : 'up'} icon="alert-triangle" />
        <KpiCard label="Curriculum"        value="71%" delta="On track" deltaDir="up" icon="book-2" />
      </div>
